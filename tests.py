        ...
        match mode: 
            ...
            case "Kr_theta/r":
               ...
               # tempGauss = Gauss(self.sigma, self.mu, self.epsilon)
                ...
                return ((3 * cot**3 * tempGauss.fprim) / self.LL**3 +
                        (3 * cot**3 * tempGauss.fprim(self.LL * tempGauss.tg(x))) / self.LL**3 -
                        (3 * cot**2 * tempGauss.fbis) / self.LL**2 -
                        (3 * cot**2 * tempGauss.fbis(self.LL * tempGauss.tg(x))) / self.LL**2 -
                        (2 * cot * tempGauss.ftris(self.LL * tempGauss.tg(x))) / self.LL +
                        (2 * cot * tempGauss.ftris(self.LL * tempGauss.tg(x))) / self.LL -
            ...
            case "Krr":
                #ERR = FF[2] u rostwora
                expression = (1 / (2 * self.LL**5)) * 3 * (-1 + 3 * u**2) * self.epsilon * cot**3 * csc**2 * (
                -3 * self.LL * np.sin(np.pi * x) * tempGauss.f(self.t - self.LL * np.tan((np.pi * x) / 2)) -
                3 * self.LL * np.sin(np.pi * x) * tempGauss.f(self.t + self.LL * np.tan((np.pi * x) / 2)) -
                3 * ERR(self.t - self.LL * np.tan((np.pi * x) / 2)) -
                3 * np.cos(np.pi * x) * ERR(self.t - self.LL * np.tan((np.pi * x) / 2)) +
                3 * ERR(self.t + self.LL * np.tan((np.pi * x) / 2)) +
                3 * np.cos(np.pi * x) * FF[2](t + self.LL * np.tan((np.pi * x) / 2)) -
                self.LL**2 * tempGauss.fprim(self.t - self.LL * np.tan((np.pi * x) / 2)) +
                self.LL**2 * np.cos(np.pi * x) * tempGauss.fprim(self.t - self.LL * np.tan((np.pi * x) / 2)) +
                self.LL**2 * tempGauss.fprim(self.t + self.LL * np.tan((np.pi * x) / 2)) -
                self.LL**2 * np.cos(np.pi * x) * tempGauss.fprim(self.t + self.LL * np.tan((np.pi * x) / 2)))
                return expression
)
